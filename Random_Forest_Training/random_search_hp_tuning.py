...
def perform_grid_search(train_data, random_grid):
    random.seed(SEED)
    np.random.seed(SEED)
    ...
    clf = RandomForestClassifier(n_estimators=1000,
                                 ...
                                 random_state = SEED,
                                 n_jobs=-1)
    rf_search = RandomizedSearchCV(estimator = clf, param_distributions = random_grid, n_iter = 20, cv = 3, verbose=2, random_state=42, n_jobs = -1)
...
def main():
    ...
    for test_year in range(2023, 2024):
        ...
        search_grid = create_random_search_grid()
        search_results = perform_grid_search(train_data, search_grid)

        # importances[test_year] = clf.feature_importances_
        labels, predictions = tester(search_results.best_estimator_, test_data)
